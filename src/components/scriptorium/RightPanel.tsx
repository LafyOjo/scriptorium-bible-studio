...
type Props = {
  ...
  onAddNote: (
    chapterId: string,
    note: { text: string; excerpt: string; theme?: HighlightTheme },
  ) => void;
...
type Tab = "notes" | "bookmarks" | "search" | "preview" | "organise";

export function RightPanel(p: Props) {
  const [tab, setTab] = useState<Tab>("preview");
  return (
    <aside className="flex max-h-[56vh] min-h-0 w-full shrink-0 flex-col overflow-hidden rounded-2xl parchment-panel lg:h-full lg:max-h-none lg:w-80">
      <div className="flex border-b border-border/60">
        {(
          [
            ...
            { k: "organise", icon: Tags, label: "Tags" },
          ] as const
        ).map((t) => (
          <button
            key={t.k}
            ...
            className={cn(
              ...
              tab === t.k
                ? "bg-ivory text-primary border-b-2 border-gold"
      ...
      <div className="flex-1 overflow-auto">
        ...
        {tab === "notes" && (
          <NotesTab
            chapter={p.activeChapter}
            selectionText={p.selectionText}
            onAdd={p.onAddNote}
            onRemove={p.onRemoveNote}
          />
        )}
        {tab === "bookmarks" && (
          <BookmarksTab
            bookmarks={p.state.bookmarks}
            chapters={p.state.chapters}
            books={p.state.books}
            activeChapter={p.activeChapter}
            activeBook={p.activeBook}
            selectionText={p.selectionText}
            onAdd={p.onAddBookmark}
            onRemove={p.onRemoveBookmark}
            onSelect={p.onSelectChapter}
          />
        )}
        {tab === "search" && <SearchTab state={p.state} onSelect={p.onSelectChapter} />}
        {tab === "organise" && (
          <OrganisationTab
            chapter={p.activeChapter}
            collections={p.state.collections}
            onUpdateChapter={p.onUpdateChapter}
          />
        )}
      </div>
  ...
  if (!chapter || !book) {
    return (
      <EmptyPanel
        ...
        body="Open a chapter to see it rendered as a manuscript page."
      />
    );
  }
  return (
    ...
    <div className="p-4 space-y-4">
      <div className="rounded-lg border border-border bg-ivory p-3">
        ...
        <div className="font-serif text-sm text-ink min-h-[2rem] italic">
          {selectionText || (
            <span className="text-muted-foreground not-italic">Select text in the editor…</span>
          )}
        </div>
          ...
          {chapter.notes.map((n) => (
            <li key={n.id} className="rounded-md border border-border bg-ivory p-2 text-xs">
              <div className="flex items-start justify-between gap-2">
                ...
                <button
                  onClick={() => onRemove(chapter.id, n.id)}
              ...
              {n.theme && (
                <span
                  className={cn(
                    "mt-1 inline-block px-1.5 py-0.5 rounded text-[10px]",
                    THEME_COLORS[n.theme].className,
                  )}
                >
...
function BookmarksTab({
  ...
  activeBook: Book | null;
  selectionText: string;
  onAdd: (b: Omit<Bookmark, "id" | "createdAt">) => void;
  ...
  return (
    <div className="p-4 space-y-4">
            ...
            <button
              onClick={() =>
                onAdd({
                  ...
                  label: `${activeBook.name} ${activeChapter.number}`,
                })
              }
              className="rounded-md border border-border bg-background px-2 py-1 text-xs"
            >
      ...
      <div>
        ...
        {bookmarks.length === 0 && (
          <div className="text-xs italic text-muted-foreground">No bookmarks yet.</div>
        )}
        <ul className="space-y-2">
          {bookmarks.map((b) => {
              ...
              <li key={b.id} className="rounded-md border border-border bg-ivory p-2 text-xs">
                <div className="flex items-start justify-between gap-2">
                  ...
                  >
                    ...
                    {b.passage && (
                      <div className="italic text-ink/70 mt-0.5 line-clamp-2">“{b.passage}”</div>
                    )}
  ...
  const results = useMemo(() => {
    ...
    return state.chapters.flatMap((c) => {
      ...
      const matchesQuery =
        ...
        `${book?.name ?? ""} ${c.number}`.toLowerCase().includes(query) ||
        `chapter ${c.number}`.includes(query) ||
        c.title.toLowerCase().includes(query) ||
        c.tags.some((t) => t.toLowerCase().includes(query)) ||
        c.notes.some(
          (n) => n.text.toLowerCase().includes(query) || n.excerpt.toLowerCase().includes(query),
        );
      const matchesTheme =
        !theme || c.html.includes(`hl-${theme}`) || c.notes.some((n) => n.theme === theme);
      ...
      <ul className="space-y-2">
        {results.map(({ chapter, book, excerpt }) => (
          <li key={chapter.id}>
            ...
            >
              <div className="font-serif text-primary text-sm">
...
function OrganisationTab({
  ...
  chapter: Chapter | null;
  collections: ScriptoriumState["collections"];
  onUpdateChapter: (chapterId: string, patch: Partial<Chapter>) => void;
}) {
  const [tag, setTag] = useState("");
  if (!chapter)
    return <EmptyPanel title="Tags & Collections" body="Open a chapter to organise it." />;

  const addTag = () => {
    const clean = tag.trim().replace(/^#/, "");
    if (!clean) return;
    const next = Array.from(new Set([...chapter.tags, clean]));
    onUpdateChapter(chapter.id, { tags: next });
    setTag("");
  };

  const removeTag = (tagToRemove: string) => {
    onUpdateChapter(chapter.id, { tags: chapter.tags.filter((t) => t !== tagToRemove) });
  };
    ...
    <div className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          {chapter.tags.length === 0 && (
            <span className="text-xs italic text-muted-foreground">
              No tags yet. Add themes like creation, exile or poetry.
            </span>
          )}
          {chapter.tags.map((t) => (
            <span
              key={t}
              className="inline-flex items-center gap-1 rounded-full bg-gold-soft/35 px-2 py-1 text-xs text-primary"
            >
              #{t}
              <button
                onClick={() => removeTag(t)}
      ...
      <div className="rounded-lg border border-border bg-ivory p-3">
        ...
        <p className="text-xs text-muted-foreground">
          Tags, notes, highlight themes, book names, chapter numbers and written text are all
...
<h2 style="text-align:center;font-weight:400">${chapter.title}</h2>${chapter.html}</body></html>`;
  ...
  const shareChapter = async () => {
    const title = `${book?.name ?? "Chapter"} ${chapter.number}: ${chapter.title}`;
    ...
    try {
      ...
      // The share sheet can be cancelled by the user; keep the interface quiet.
    }
  };
  ...
  return (
    <div className="border-t border-border/60 bg-ivory/70 px-3 py-2">
      ...
      <div className="grid grid-cols-5 gap-1">
        ...
        >
          <Share2 className="h-3 w-3" /> {shareState === "copied" ? "Copied" : "Share"}
        </button>
        <button
          title="Plain text"
          onClick={() => download(`${base}.txt`, htmlToPlainText(chapter.html), "text/plain")}
          className="inline-flex items-center justify-center gap-1 rounded-md border border-border bg-background py-1 text-[10px] hover:bg-gold-soft/40"
        >
        ...
        <button
          title="HTML"
          onClick={() => download(`${base}.html`, html, "text/html")}
          className="inline-flex items-center justify-center gap-1 rounded-md border border-border bg-background py-1 text-[10px] hover:bg-gold-soft/40"
        >
          <FileText className="h-3 w-3" /> HTML
        </button>
        <button
          title="JSON backup of all data"
          onClick={() =>
            download(`scriptorium-backup.json`, JSON.stringify(state, null, 2), "application/json")
          }
          className="inline-flex items-center justify-center gap-1 rounded-md border border-border bg-background py-1 text-[10px] hover:bg-gold-soft/40"
        >
